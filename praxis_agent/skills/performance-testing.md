...
6. **After a code fix** (`code_edit` + rebuild via `start_service`), repeat the exact same load
   + sampling window and compare the new time series against the earlier one to confirm the
   improvement, rather than eyeballing a single snapshot.
